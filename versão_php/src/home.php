...
<body id="zoombody">
  <header id="top">
    ...
    <section class="section-search">
      <label style="position: absolute; z-index: -1; color: transparent;" for="search">Barra de pesquisa do site</label>
  ...
  <main>
    <section class="section-vacancy">
      <h1 class="main-title">Vagas em Alta 🔥</h1>
      <?php
            $sql = "SELECT * FROM  tb_vagas ORDER BY id DESC LIMIT 3";
            $res = $conn->query($sql);
            $qp =  $res->num_rows;

             if ($qp > 0) {

              print '<div class="container-cards">';
              ...
              while ($row = $res->fetch_object()) {
                ...
                print '
                <a href="' . $row->link_vaga . '">
                  <div class="card card-vacancy">
                  <div class="card-header">
                    <img src="http://marcusmarques.com.br/wp-content/uploads/2017/03/logotipo.jpg" alt="Imagem ilustrativa referente a empresa que disponibilizou a vaga." />
        
                    <div class="primary-text-card">
                      <p class="text-card text-card-bold">' . $row->nome_vaga . '</p>
                      <p class="text-card text-card-bold second">R$ ' . $row->salario_vaga . '</p>
                    </div>
                  </div>
        
                  <p class="text-card description">' . mb_strimwidth($row->descricao_vaga, 0, 55, '...') . '</p>
        
                  <p class="text-card">
                    <span class="text-card-bold">Nível: </span>' . mb_strimwidth($row->nivel_vaga, 0, 20, '...') .  '
                  </p>
                  <p class="text-card">
                    <span class="text-card-bold">Horário: </span>' . $row->horario_vaga . '
                  </p>
                  <p class="text-card">
                    <span class="text-card-bold">Contato: </span>' . $row->contato_vaga . '
                  </p>
                </div>
              </a>
              ';
            ...
            <div class="container-cards">  
              <div class="card card-vacancy">
                <div class="card-header">
                  ...
                  <div class="primary-text-card">
                    <p class="text-card text-card-bold">Programador</p>
                    <p class="text-card text-card-bold second">R$ 3.000,00</p>
                  </div>
                </div>

                <p class="text-card description">Descrição Simples da vaga.</p>

                <p class="text-card">
                  <span class="text-card-bold">Nível: </span>Júnior
                </p>
                <p class="text-card">
                  <span class="text-card-bold">Horário: </span>12h - 19h
                </p>
                <p class="text-card">
                  <span class="text-card-bold">Contato: </span>(11) 99873-4100
                </p>
              </div>
              <div class="card card-vacancy">
                <div class="card-header">
    ...
    <section class="section-couse">
      <h2 class="main-title">Cursos em Alta 🔥</h2>
      <?php
       $sql = "SELECT * FROM  tb_cursos ORDER BY id DESC LIMIT 3";
       $res = $conn->query($sql);
       $qp =  $res->num_rows;
        ...
        if ($qp > 0) {
         ...
         while ($row = $res->fetch_object()) {
          ...
          print '
          <a href="' . $row->link_curso . '">
            <div class="card card-course">
                <p class="text-card text-card-bold">' . $row->nome_curso  . '</p>
                <p class="text-card text-card-bold second">' . $row->fornecedor_curso . '</p>

                <p class="text-card description">' . mb_strimwidth($row->descricao_curso, 0, 55, '...') . '</p>

                <p class="text-card">
                  <span class="text-card-bold">Nível: </span>' . mb_strimwidth($row->nivel_curso, 0, 20, '...') . '
                </p>
                <p class="text-card">
                  <span class="text-card-bold">Horário: </span>' . $row->horario_curso . '
                </p>
                <p class="text-card">
                  <span class="text-card-bold">Contato: </span>' . $row->contato_curso . '
                </p>
            </div>
          </a>
          ';

           // print "<p>" .$row->nome_curso  . "</p>";
           // print "<p>" .$row->fornecedor_curso . "</p>";
           // print "<p>" . $row->descricao_curso . "</p>";
           // print "<p>" .$row->nivel_curso . "</p>";
           // print "<p>" . $row->horario_curso . "</p>";
           // print "<p>" . $row->contato_curso . "</p>";
    ...
    <section class="section-blog">
      <h3 class="main-title">Nosso Blog</h3>

      <div class="container-blog">
        <div class="text-button-blog">
          <p class="text-resume-blog">
          ...
          <a href="./publicacoes-blog.php">Ler mais</a>
        </div>

        <img src="./img/image-home-blog.svg" alt="Imagem ilustrativa, de uma mulher feliz lendo um blog, enquanto toma o seu café" />
      </div>
    </section>
      ...
      <a href="./contato.php">
        <form>
          <div class="form-container">
            ...
            <input type="text" id="nome" name="nome" disabled="disabled" required />

            <label for="email">E-mail</label>
            <input type="email" id="email" name="email" disabled="disabled" required />

            <label for="mensagem">Sua mensagem</label>
            <textarea name="mensagem" id="mensagem" disabled="disabled" required></textarea>
          </div>
        </form>
      </a>
    </section>
    ...
    <section class="section-team">
      <h3 class="main-title">Nossa Equipe 🙅‍♂️</h3>

      <div class="container-team">
        <a href="./equipe.php">
          <img src="./img/alexandre.svg" alt="Imagem do integrante Alexandre" />
        </a>
        <a href="./equipe.php">
          <img src="./img/enrico.svg" alt="Imagem do integrante Enrico" />
        </a>
        <a href="./equipe.php">
          <img src="./img/isabelle.svg" alt="Imagem do integrante Isabelle" />
        </a>
      </div>
    </section>
  </main>
