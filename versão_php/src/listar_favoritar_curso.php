<!-- Pagina para listar os cursos favoritandos do usuario  -->
...
<head>
  ...
  <title>Cursos favoritos - Accessy</title>
...
<body id="zoombody">
  ...
  <main>
    <div class="campo-curso campo-vagas">

      <?php
      include("processamento/conexao.php");

      $login = "SELECT favoritar_curso FROM tb_pessoa WHERE id =" . $_SESSION["id_usuario"];
      $res = $conn->query($login);
      $row = $res->fetch_object();
      $qp = explode(",", $row->favoritar_curso);

      if ($qp[0] != '') {
        $i = 0;
        while ($i < count($qp)) {
          $id_curso = $qp[$i];
          $todos_favoritados = "SELECT * FROM tb_cursos WHERE id =  $id_curso";
          ...
          print '
                ...
                <div class="favorite-button">
                  <a title="Desfavoritar curso" href="./processamento/processa_desfavoritar_cursos.php?id_curso=' . $row->id . '">
